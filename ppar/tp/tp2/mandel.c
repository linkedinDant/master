...
int main(int argc, char *argv[]) {
  ...
  fin = my_gettimeofday();
  printf("Temps du processus %d : %g sec\n", rank, fin - debut);
  
  if(rank != 0) {
    MPI_Send(grid, w * hlocal, MPI_UNSIGNED_CHAR, 0, 0, MPI_COMM_WORLD);
  } else {    
    for(i = 1 ; i < size ; i++) {
      MPI_Status status;
      MPI_Recv(grid + (i * w * hlocal), w * hlocal, MPI_UNSIGNED_CHAR, i, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
    }  
    sauver_rasterfile( "mandel_par.ras", w, h, grid);
  }
  MPI_Finalize();
  return 0;
}
